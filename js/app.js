...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const screens = {
    ...
    hub:         document.getElementById('screen-modalidades') || document.getElementById('screen-hub'),
    modalidades: document.getElementById('screen-modalidades'),
  ...
  btnPracticeRoulette?.addEventListener('click', () => showScreen('modalidades'));

  // ============================================================
  //  CENTRO VIAL (HUB): JUEGOS, SIMULADORES Y MANUALES
  // ============================================================

  // 1. Hub Category Filtering
  const hubPillBtns = document.querySelectorAll('.hub-pill-btn');
  const hubSections = {
    games: document.getElementById('hub-section-games'),
    simulators: document.getElementById('hub-section-simulators'),
    manuals: document.getElementById('hub-section-manuals')
  };

  hubPillBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      hubPillBtns.forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      const filter = btn.dataset.hubFilter;

      if (filter === 'all') {
        Object.values(hubSections).forEach(sec => { if (sec) sec.style.display = 'flex'; });
      } else {
        Object.entries(hubSections).forEach(([key, sec]) => {
          if (sec) sec.style.display = (key === filter) ? 'flex' : 'none';
        });
      }
    });
  });

  // 2. Hub Quick Action Triggers (Juegos)
  const btnHubQuickRoulette = document.getElementById('btn-hub-quick-roulette');
  const btnHubRoulette = document.getElementById('btn-hub-roulette');
  const btnHubTrivia = document.getElementById('btn-hub-trivia');
  const btnHubSignals = document.getElementById('btn-hub-signals');

  btnHubQuickRoulette?.addEventListener('click', () => showScreen('roulette'));
  btnHubRoulette?.addEventListener('click', () => showScreen('roulette'));

  btnHubTrivia?.addEventListener('click', () => {
    startQuizRound({ id: 'general', label: 'TRIVIA EXPRÉS', name: 'Desafío Rápido', icon: '⚡', color: '#8DE2D6' });
  });

  btnHubSignals?.addEventListener('click', () => {
    startQuizRound({ id: 'senales', label: 'SEÑALÉTICA', name: 'Identificador de Señales', icon: '🛑', color: '#EF4444' });
  });

  // ============================================================
  //  SIMULADOR 1: TIEMPO DE REACCIÓN
  // ============================================================
  const modalSimReaction    = document.getElementById('modal-sim-reaction');
  const btnCloseSimReaction = document.getElementById('btn-close-sim-reaction');
  const btnOpenSimReaction  = document.getElementById('btn-open-sim-reaction');
  const simLampRed          = document.getElementById('sim-lamp-red');
  const simLampYellow       = document.getElementById('sim-lamp-yellow');
  const simLampGreen        = document.getElementById('sim-lamp-green');
  const simReactionStatus   = document.getElementById('sim-reaction-status');
  const btnSimPedal         = document.getElementById('btn-sim-pedal');
  const simPedalIcon        = document.getElementById('sim-pedal-icon');
  const simPedalLabel       = document.getElementById('sim-pedal-label');
  const simReactionResult   = document.getElementById('sim-reaction-result');
  const simMsVal            = document.getElementById('sim-ms-val');
  const simGradeBadge       = document.getElementById('sim-grade-badge');
  const simPedagogicTip     = document.getElementById('sim-pedagogic-tip');
  const btnSimReactionRetry = document.getElementById('btn-sim-reaction-retry');

  let reactionTestState = 'idle'; // 'idle', 'waiting', 'ready', 'finished'
  ...
  function resetReactionTest() {
    ...
    if (simLampRed) simLampRed.className = 'traffic-lamp red';
    if (simLampYellow) simLampYellow.className = 'traffic-lamp yellow';
    if (simLampGreen) simLampGreen.className = 'traffic-lamp green';
    if (simReactionStatus) simReactionStatus.textContent = 'Presiona "INICIAR PRUEBA" para comenzar';
    if (simReactionResult) simReactionResult.style.display = 'none';
    if (btnSimPedal) {
      btnSimPedal.className = 'btn-sim-pedal idle';
      if (simPedalIcon) simPedalIcon.textContent = 'play_arrow';
      if (simPedalLabel) simPedalLabel.textContent = 'INICIAR PRUEBA';
    }
  }

  btnOpenSimReaction?.addEventListener('click', () => {
    resetReactionTest();
    if (modalSimReaction) {
      modalSimReaction.style.display = 'flex';
      modalSimReaction.classList.add('active');
    }
  });

  btnCloseSimReaction?.addEventListener('click', () => {
    clearTimeout(reactionTimeoutId);
    if (modalSimReaction) {
      modalSimReaction.style.display = 'none';
      modalSimReaction.classList.remove('active');
    }
  });

  btnSimReactionRetry?.addEventListener('click', () => {
    resetReactionTest();
  ...
  function startReactionCountdown() {
    ...
    if (simLampRed) simLampRed.className = 'traffic-lamp red lit';
    if (simLampYellow) simLampYellow.className = 'traffic-lamp yellow lit';
    if (simLampGreen) simLampGreen.className = 'traffic-lamp green';
    if (simReactionStatus) simReactionStatus.textContent = '🔴🟡 ¡Preparado! Aguarda a que cambie a verde...';
    if (simReactionResult) simReactionResult.style.display = 'none';

    if (btnSimPedal) {
      btnSimPedal.className = 'btn-sim-pedal waiting';
      if (simPedalIcon) simPedalIcon.textContent = 'hourglass_top';
      if (simPedalLabel) simPedalLabel.textContent = '¡ESPERA EL VERDE!';
    }
    ...
    reactionTimeoutId = setTimeout(() => {
      ...
      if (simLampRed) simLampRed.className = 'traffic-lamp red';
      if (simLampYellow) simLampYellow.className = 'traffic-lamp yellow';
      if (simLampGreen) simLampGreen.className = 'traffic-lamp green lit';
      if (simReactionStatus) simReactionStatus.textContent = '🟢 ¡¡VERDE!! ¡PISA EL PEDAL YA!';

      if (btnSimPedal) {
        btnSimPedal.className = 'btn-sim-pedal ready';
        if (simPedalIcon) simPedalIcon.textContent = 'touch_app';
        if (simPedalLabel) simPedalLabel.textContent = '¡FRENA AHORA!';
      }

      if (audioSystem && audioSystem.playCorrect) audioSystem.playCorrect();
    }, randomDelay);
  }

  btnSimPedal?.addEventListener('click', () => {
    if (reactionTestState === 'idle') {
      ...
      if (simLampYellow) simLampYellow.className = 'traffic-lamp yellow lit';
      if (simLampGreen) simLampGreen.className = 'traffic-lamp green';
      if (simReactionStatus) simReactionStatus.textContent = '⚠️ ¡Salida en falso! Presionaste antes del verde.';
      if (btnSimPedal) {
        btnSimPedal.className = 'btn-sim-pedal idle';
        if (simPedalIcon) simPedalIcon.textContent = 'warning';
        if (simPedalLabel) simPedalLabel.textContent = 'REINTENTAR';
      }
      if (audioSystem && audioSystem.playWrong) audioSystem.playWrong();
    } else if (reactionTestState === 'ready') {
      const elapsed = Math.round(performance.now() - reactionStartTime);
      reactionTestState = 'finished';
      if (simReactionStatus) simReactionStatus.textContent = `✅ ¡Excelente! Tu tiempo fue de ${elapsed} ms`;
      if (btnSimPedal) {
        btnSimPedal.className = 'btn-sim-pedal idle';
        if (simPedalIcon) simPedalIcon.textContent = 'check_circle';
        if (simPedalLabel) simPedalLabel.textContent = 'PRUEBA FINALIZADA';
      }

      if (simReactionResult) simReactionResult.style.display = 'flex';
      if (simMsVal) simMsVal.textContent = `${elapsed} ms`;

      let grade = '⚡ Reflejos Óptimos';
      let gradeColor = 'var(--success)';
      if (elapsed < 230) {
        grade = '🏆 Reflejos de Piloto Profesional';
        gradeColor = 'var(--secondary-container)';
      } else if (elapsed <= 330) {
        grade = '⚡ Reflejos Muy Buenos';
        gradeColor = 'var(--success)';
      } else if (elapsed <= 450) {
        grade = '🚗 Reflejos Promedio Normal';
        gradeColor = 'var(--tertiary)';
      } else {
        grade = '⚠️ Reflejos Lentos / Posible Distracción';
        gradeColor = 'var(--error)';
      }

      if (simGradeBadge) {
        simGradeBadge.textContent = grade;
        simGradeBadge.style.color = gradeColor;
      }

      const metersAt60 = ((60 / 3.6) * (elapsed / 1000)).toFixed(1);
      if (simPedagogicTip) {
        simPedagogicTip.innerHTML = `A 60 km/h en ciudad, recorriste <strong>${metersAt60} metros</strong> a ciegas antes de que tu pie tocara el pedal de freno.`;
      }
      if (audioSystem && audioSystem.playSpinEnd) audioSystem.playSpinEnd();
    }
  });

  // ============================================================
  //  SIMULADOR 2: DISTANCIA DE FRENADO
  // ============================================================
  const modalSimBraking    = document.getElementById('modal-sim-braking');
  const btnCloseSimBraking = document.getElementById('btn-close-sim-braking');
  const btnOpenSimBraking  = document.getElementById('btn-open-sim-braking');
  const simSpeedRange      = document.getElementById('sim-speed-range');
  const simSpeedVal        = document.getElementById('sim-speed-val');
  const simTrackReacM      = document.getElementById('sim-track-reac-m');
  const simTrackBrakeM     = document.getElementById('sim-track-brake-m');
  const simTrackReacFill   = document.getElementById('sim-track-reac-fill');
  const simTrackBrakeFill  = document.getElementById('sim-track-brake-fill');
  const simTotalMeters     = document.getElementById('sim-total-meters');
  const simCarEquivalents  = document.getElementById('sim-car-equivalents');
  const simBrakingTip      = document.getElementById('sim-braking-tip');

  let simSelectedWeather = 'seco';
  let simSelectedVehicle = 'auto';

  function calculateBrakingSimulation() {
    const speed = parseInt(simSpeedRange?.value || '60', 10);
    if (simSpeedVal) simSpeedVal.textContent = `${speed} km/h`;
    ...
    let mu = 0.75;
    if (simSelectedWeather === 'mojado') mu = 0.40;
    if (simSelectedWeather === 'hielo') mu = 0.15;

    let vehFactor = 1.0;
    if (simSelectedVehicle === 'moto') vehFactor = 1.12;
    if (simSelectedVehicle === 'colectivo') vehFactor = 1.35;

    const g = 9.81;
    const dBrake = ((v * v) / (2 * mu * g)) * vehFactor;
    const dTotal = dReac + dBrake;

    if (simTrackReacM) simTrackReacM.textContent = `${dReac.toFixed(1)}m`;
    if (simTrackBrakeM) simTrackBrakeM.textContent = `${dBrake.toFixed(1)}m`;

    const reacPct = Math.max(15, Math.min(85, (dReac / dTotal) * 100));
    const brakePct = 100 - reacPct;

    if (simTrackReacFill) simTrackReacFill.style.width = `${reacPct}%`;
    if (simTrackBrakeFill) simTrackBrakeFill.style.width = `${brakePct}%`;

    if (simTotalMeters) simTotalMeters.textContent = `${dTotal.toFixed(1)} metros`;

    const carCount = Math.max(1, Math.round(dTotal / 4.2));
    if (simCarEquivalents) {
      simCarEquivalents.innerHTML = `🚗 Aprox. <strong>${carCount} autos</strong> en fila`;
    }

    if (simBrakingTip) {
      if (simSelectedWeather === 'hielo') {
        simBrakingTip.innerHTML = `❄️ <strong>Peligro Extremo:</strong> Con hielo la distancia de frenado se multiplica casi por 5. ¡Reduce la velocidad al mínimo y no hagas maniobras bruscas!`;
      } else if (simSelectedWeather === 'mojado') {
        simBrakingTip.innerHTML = `🌧️ <strong>Asfalto Mojado:</strong> La adherencia disminuye un 40%. Aumenta la distancia de seguridad a <strong>4 segundos</strong> para evitar choques en cadena.`;
      } else {
        simBrakingTip.innerHTML = `💡 <strong>Regla de los 2 Segundos:</strong> En pavimento seco mantén al menos 2 segundos de separación con el vehículo que te antecede.`;
      }
    }
  }

  btnOpenSimBraking?.addEventListener('click', () => {
    calculateBrakingSimulation();
    if (modalSimBraking) {
      modalSimBraking.style.display = 'flex';
      modalSimBraking.classList.add('active');
    }
  });

  btnCloseSimBraking?.addEventListener('click', () => {
    if (modalSimBraking) {
      modalSimBraking.style.display = 'none';
      modalSimBraking.classList.remove('active');
    }
  });

  simSpeedRange?.addEventListener('input', calculateBrakingSimulation);

  document.querySelectorAll('#sim-weather-pills .sim-choice-pill').forEach(btn => {
    btn.addEventListener('click', () => {
      document.querySelectorAll('#sim-weather-pills .sim-choice-pill').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      simSelectedWeather = btn.dataset.weather || 'seco';
      calculateBrakingSimulation();
    });
  });

  document.querySelectorAll('#sim-vehicle-pills .sim-choice-pill').forEach(btn => {
    btn.addEventListener('click', () => {
      document.querySelectorAll('#sim-vehicle-pills .sim-choice-pill').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      simSelectedVehicle = btn.dataset.veh || 'auto';
      calculateBrakingSimulation();
    });
  });

  // ============================================================
  //  SIMULADOR 3: PUNTOS CIEGOS
  // ============================================================
  const modalSimBlindspot    = document.getElementById('modal-sim-blindspot');
  const btnCloseSimBlindspot = document.getElementById('btn-close-sim-blindspot');
  const btnOpenSimBlindspot  = document.getElementById('btn-open-sim-blindspot');
  const btnBlindColectivo    = document.getElementById('btn-blind-colectivo');
  const btnBlindAuto         = document.getElementById('btn-blind-auto');
  const blindZoneTitle       = document.getElementById('blind-zone-title');
  const blindZoneDesc        = document.getElementById('blind-zone-desc');
  const blindspotVehBox      = document.getElementById('blindspot-vehicle-box');

  const BLINDSPOT_DATA = {
    colectivo: {
      front: {
        title: '⚠️ Zona Frontal Baja (Colectivo)',
        desc: 'El chofer está sentado en altura. Los primeros 2 a 3 metros inmediatamente delante del paragolpes delantero son invisibles. Si un peatón o ciclista cruza pegado al frente, el chofer no lo verá.'
      },
      right: {
        title: '⚠️ Lateral Derecho: El Punto Más Crítico (Colectivo)',
        desc: 'Por el largo del colectivo (12 a 18 metros) y la distancia al espejo derecho, este sector tiene el ángulo muerto más amplio. Si una moto o bici adelanta por la derecha en un giro, el impacto es inminente.'
      },
      left: {
        title: '⚠️ Lateral Izquierdo (Colectivo)',
        desc: 'Aunque el chofer tiene el espejo izquierdo más cercano, los vehículos pequeños ubicados a la altura de la rueda trasera quedan fuera del campo visual directo.'
      },
      rear: {
        title: '⚠️ Parte Trasera Completa (Colectivo)',
        desc: 'Los colectivos carecen de espejo retrovisor interior. Todo lo que esté detrás del coche a menos de 10 metros es totalmente ciego para el chofer salvo que cuente con cámara de retroceso.'
      }
    },
    auto: {
      front: {
        title: '⚠️ Ángulo de los Pilares Delanteros (Pilar A)',
        desc: 'Los parantes que sostienen el parabrisas ocultan peatones y motos en cruces peatonales y rotondas. Siempre es necesario balancear la cabeza para mirar detrás del pilar.'
      },
      right: {
        title: '⚠️ Ángulo Muerto Trasero Derecho (Auto)',
        desc: 'La zona lateral trasera derecha es clásica en cambios de carril. Un vehículo que circula allí no se ve en el espejo interior ni en el lateral sin girar levemente el cuello.'
      },
      left: {
        title: '⚠️ Ángulo Muerto Trasero Izquierdo (Auto)',
        desc: 'Común en autopistas y avenidas al sobrepasar. Ajustar los espejos bien abiertos hacia afuera minimiza drásticamente este punto ciego.'
      },
      rear: {
        title: '⚠️ Zona Baja Trasera (Auto)',
        desc: 'Al dar marcha atrás, niños, mascotas o postes bajos quedan por debajo de la línea del baúl o luneta trasera.'
      }
    }
  };

  let currentBlindVehicle = 'colectivo';

  btnOpenSimBlindspot?.addEventListener('click', () => {
    if (modalSimBlindspot) {
      modalSimBlindspot.style.display = 'flex';
      modalSimBlindspot.classList.add('active');
    }
  });

  btnCloseSimBlindspot?.addEventListener('click', () => {
    if (modalSimBlindspot) {
      modalSimBlindspot.style.display = 'none';
      modalSimBlindspot.classList.remove('active');
    }
  });

  btnBlindColectivo?.addEventListener('click', () => {
    currentBlindVehicle = 'colectivo';
    btnBlindColectivo.classList.add('active');
    btnBlindAuto?.classList.remove('active');
    if (blindspotVehBox) {
      blindspotVehBox.innerHTML = `
        <span class="material-symbols-outlined text-[38px]">directions_bus</span>
        <span style="font-size:11px;font-weight:900;">COLECTIVO</span>
      `;
    }
    updateBlindspotExplanation('right');
  });

  btnBlindAuto?.addEventListener('click', () => {
    currentBlindVehicle = 'auto';
    btnBlindAuto.classList.add('active');
    btnBlindColectivo?.classList.remove('active');
    if (blindspotVehBox) {
      blindspotVehBox.innerHTML = `
        <span class="material-symbols-outlined text-[38px]">directions_car</span>
        <span style="font-size:11px;font-weight:900;">AUTO</span>
      `;
    }
    updateBlindspotExplanation('right');
  });

  function updateBlindspotExplanation(zoneKey) {
    const data = BLINDSPOT_DATA[currentBlindVehicle]?.[zoneKey];
    if (!data) return;
    if (blindZoneTitle) blindZoneTitle.textContent = data.title;
    if (blindZoneDesc) blindZoneDesc.textContent = data.desc;
  }

  document.querySelectorAll('.blindspot-zone').forEach(el => {
    el.addEventListener('click', () => {
      const zone = el.dataset.zone;
      updateBlindspotExplanation(zone);
    });
  });

  // ============================================================
  //  MANUALES VIALES DIDÁCTICOS (VISOR INTERACTIVO)
  // ============================================================
  const modalManualViewer = document.getElementById('modal-manual-viewer');
  const btnCloseManualView = document.getElementById('btn-close-manual-view');
  const manualViewTag = document.getElementById('manual-view-tag');
  const manualViewTitle = document.getElementById('manual-view-title');
  const manualChaptersNav = document.getElementById('manual-chapters-nav');
  const manualViewContent = document.getElementById('manual-view-content');

  const MANUALS_DATABASE = {
    moto: {
      tag: 'CLASE A • MOTOS',
      title: 'Manual de Conducción Segura de Motovehículos',
      chapters: [
        {
          id: 'ch1',
          title: '1. Equipamiento Homologado',
          content: `
            <h4>🪖 Uso Obligatorio de Casco Integral o Modular</h4>
            <p>El casco es el elemento primordial de seguridad pasiva. Reduce en un 70% las probabilidades de traumatismos craneales graves y en un 40% la mortalidad en siniestros viales.</p>
            <ul>
              <li><strong>Talla justa:</strong> El casco no debe oscilar al mover la cabeza.</li>
              <li><strong>Abrochado siempre:</strong> Un casco sin abrochar sale despedido en el primer impacto.</li>
              <li><strong>Visor transparente:</strong> Limpio y sin rayas para conducción nocturna.</li>
            </ul>
            <div class="manual-tip-box">
              🛡️ <strong>Regla Vital:</strong> El casco vence a los 3 a 5 años o inmediatamente tras sufrir un golpe significativo, aunque no tenga roturas visibles.
            </div>
          `
        },
        {
          id: 'ch2',
          title: '2. Técnica de Frenado 70/30',
          content: `
            <h4>🛑 El Secreto del Reparto de Frenada</h4>
            <p>Al frenar, el peso de la motocicleta se transfiere bruscamente hacia la rueda delantera (transferencia de masa). Por eso:</p>
            <ul>
              <li><strong>70% en el Freno Delantero:</strong> Proporciona la mayor potencia de detención.</li>
              <li><strong>30% en el Freno Trasero:</strong> Estabiliza la moto y evita el derrape lateral.</li>
              <li><strong>Frenada con la moto recta:</strong> Nunca frenes bruscamente con la dirección doblada o inclinada.</li>
            </ul>
            <div class="manual-tip-box">
              ⚠️ <strong>Si la rueda delantera se bloquea:</strong> Libera inmediatamente la maneta de freno y vuelve a presionar de forma progresiva para recuperar la dirección.
            </div>
          `
        },
        {
          id: 'ch3',
          title: '3. Visibilidad y Ángulos Muertos',
          content: `
            <h4>👀 Hazte Visible en Todo Momento</h4>
            <p>La principal causa de accidentes urbanos moto-auto es: <em>"No vi a la moto"</em>.</p>
            <ul>
              <li><strong>Luces bajas siempre encendidas:</strong> Obligatorio por ley las 24 horas.</li>
              <li><strong>Ubicación en el carril:</strong> Circula en el tercio del carril más visible (centro o izquierda) y nunca pegado a los cordones.</li>
              <li><strong>No zigzaguear entre vehículos en movimiento:</strong> Maniobra de alto riesgo penada por la ley.</li>
            </ul>
          `
        },
        {
          id: 'ch4',
          title: '4. Conducción con Lluvia',
          content: `
            <h4>🌧️ Lluvia y Calzada Resbaladiza</h4>
            <p>Los primeros 10 minutos de llovizna son los más peligrosos, ya que el agua se mezcla con el polvo y aceite acumulado en el pavimento formando un barrillo jabonoso.</p>
            <ul>
              <li><strong>Cuidado con la pintura vial:</strong> Las sendas peatonales y líneas demarcatorias son extremadamente resbaladizas cuando están mojadas.</li>
              <li><strong>Evitar tapas de alcantarilla y vías de tranvía/tren.</strong></li>
              <li><strong>Duplicar la distancia de seguimiento:</strong> De 2 a 4 segundos.</li>
            </ul>
          `
        }
      ]
    ...
    colectivo: {
      tag: 'CLASE D1 • TRANSPORTE',
      title: 'Manual de Conducción Profesional y Pasajeros',
      chapters: [
        {
          id: 'ch1',
          title: '1. Paradas y Pasajeros',
          content: `
            <h4>👥 Protocolo en Paradas de Pasajeros</h4>
            <p>El conductor profesional tiene a su cargo vidas humanas y debe extremar la suavidad de las maniobras.</p>
            <ul>
              <li><strong>Arrime seguro:</strong> El vehículo debe detenerse a menos de 30 centímetros del cordón de la vereda.</li>
              <li><strong>Prohibido arrancar con puertas abiertas:</strong> Asegúrate de que el pasajero completó el ascenso/descenso antes de reanudar la marcha.</li>
              <li><strong>Espera a personas mayores o con movilidad reducida:</strong> No aceleres hasta que se hayan sentado o tomado firmemente del pasamanos.</li>
            </ul>
          `
        },
        {
          id: 'ch2',
          title: '2. Metrobús y Carril Exclusivo',
          content: `
            <h4>🚌 Circulación en Carriles Confinados</h4>
            <ul>
              <li><strong>Velocidad máxima:</strong> En carriles exclusivos y Metrobús no se deben superar los 40 a 50 km/h según la traza.</li>
              <li><strong>Prioridad absoluta al peatón:</strong> En los cruces señalizados antes de las estaciones intermedias.</li>
              <li><strong>Distancia entre unidades:</strong> Mantener al menos la longitud de un colectivo completo entre coches.</li>
            </ul>
          `
        },
        {
          id: 'ch3',
          title: '3. Alcohol Cero y Descanso',
          content: `
            <h4>⚖️ Responsabilidad Profesional y Ley 24.449</h4>
            <p>Para la Categoría D1 (Transporte de Pasajeros), la tolerancia de alcohol en sangre ha sido históricamente <strong>0,0 g/l</strong> con sanciones penales agravadas.</p>
            <ul>
              <li><strong>Régimen de descanso obligatorio:</strong> Respetar las horas de corte para evitar la fatiga y el microsueño.</li>
              <li><strong>Prohibición de celulares:</strong> Terminantemente prohibido el uso de teléfono celular o auriculares durante la conducción del servicio.</li>
            </ul>
          `
        }
      ]
    ...
    auto: {
      tag: 'CLASE B • AUTOMÓVILES',
      title: 'Manual Nacional del Conductor Particular',
      chapters: [
        {
          id: 'ch1',
          title: '1. Prioridades de Paso',
          content: `
            <h4>🚸 La Regla de la Derecha y sus Excepciones</h4>
            <p>Todo conductor debe ceder siempre el paso en las encrucijadas al que cruza desde su <strong>DERECHA</strong>. Esta prioridad solo se pierde ante:</p>
            <ul>
              <li>Señalización específica en contrario (Señal de PARE o CEDA EL PASO).</li>
              <li>Vehículos de emergencia en servicio urgente (ambulancias, bomberos, policías).</li>
              <li>Vehículos que circulan por una rotonda (quien circula adentro tiene prioridad).</li>
              <li>Peatones que cruzan lícitamente por la senda peatonal.</li>
              <li>Pasaje de una calle de tierra a una calle pavimentada.</li>
            </ul>
          `
        },
        {
          id: 'ch2',
          title: '2. Velocidades Permitidas',
          content: `
            <h4>⚡ Límites Máximos en Territorio Nacional</h4>
            <ul>
              <li><strong>Calles urbanas:</strong> 40 km/h.</li>
              <li><strong>Avenidas:</strong> 60 km/h.</li>
              <li><strong>Autovías y Rutas:</strong> 110 km/h para autos particulares.</li>
              <li><strong>Autopistas:</strong> 130 km/h.</li>
              <li><strong>Proximidad de escuelas, hospitales y pasos a nivel:</strong> 20 km/h.</li>
            </ul>
          `
        },
        {
          id: 'ch3',
          title: '3. Retención Infantil (SRI)',
          content: `
            <h4>👶 Seguridad de los Más Chicos</h4>
            <p>Los menores de 10 años deben viajar obligatoriamente en el asiento trasero utilizando el Sistema de Retención Infantil (SRI) correspondiente a su peso y estatura homologado.</p>
            <ul>
              <li><strong>Hasta los 2 años:</strong> Mirando hacia atrás (contramarcha) para proteger el cuello.</li>
              <li><strong>Cinturón de seguridad:</strong> Cada pasajero debe llevar su propio cinturón inercial abrochado.</li>
            </ul>
          `
        }
      ]
    ...
    senales: {
      tag: 'SISTEMA VIAL • SEÑALÉTICA',
      title: 'Catálogo Oficial de Señalización Vial',
      chapters: [
        {
          id: 'ch1',
          title: '1. Señales Reglamentarias (Rojas)',
          content: `
            <h4>🛑 Obligaciones, Prohibiciones y Restricciones</h4>
            <p>Tienen forma circular con orla roja y fondo blanco (excepto PARE que es octogonal y CEDA EL PASO que es triangular invertida).</p>
            <ul>
              <li><strong>Violación:</strong> Constituye falta grave sancionable con multa y retención de puntos.</li>
              <li><strong>Ejemplos clave:</strong> Prohibido girar, Contramano, Velocidad Máxima, Estacionamiento prohibido.</li>
            </ul>
          `
        },
        {
          id: 'ch2',
          title: '2. Señales Preventivas (Amarillas)',
          content: `
            <h4>⚠️ Advertencias de Peligro en la Vía</h4>
            <p>Tienen forma de rombo con fondo amarillo y símbolos negros. Anticipan una condición peligrosa para que el conductor reduzca la velocidad.</p>
            <ul>
              <li>Curva y contracurva peligrosa.</li>
              <li>Cruce ferroviario sin barrera (Cruz de San Andrés).</li>
              <li>Estrechamiento de calzada o pendiente pronunciada.</li>
            </ul>
          `
        },
        {
          id: 'ch3',
          title: '3. Señales Informativas y Transitorias',
          content: `
            <h4>ℹ️ Orientación y Obras</h4>
            <ul>
              <li><strong>Informativas (Azules / Verdes):</strong> Brindan datos útiles de destinos, kilometrajes, servicios mecánicos, hospitales y peajes.</li>
              <li><strong>Transitorias (Naranjas):</strong> Señalizan zonas de obras viales. Prevalecen sobre las demás señalizaciones fijas mientras duren los trabajos.</li>
            </ul>
          `
        }
      ]
    ...
    ley: {
      tag: 'LEGISLACIÓN • NORMAS',
      title: 'Ley Nacional de Tránsito N° 24.449 y Ley de Alcohol Cero',
      chapters: [
        {
          id: 'ch1',
          title: '1. Ley de Alcohol Cero',
          content: `
            <h4>🍷 Tolerancia Cero al Volante</h4>
            <p>La Ley Nacional establece <strong>0,0 miligramos de alcohol por litro de sangre</strong> para conductores de todo tipo de vehículos a motor en la vía pública.</p>
            <ul>
              <li>No existe margen de "una copa". El alcohol deteriora los reflejos desde el primer trago.</li>
              <li>La negativa a realizar el control de alcoholemia presume culpabilidad y equivale a dar positivo.</li>
            </ul>
          `
        },
        {
          id: 'ch2',
          title: '2. Documentación Obligatoria',
          content: `
            <h4>📋 Qué debes llevar siempre contigo:</h4>
            <ul>
              <li>Documento Nacional de Identidad (DNI físico o digital habilitado).</li>
              <li>Licencia Nacional de Conducir vigente y acorde a la clase del vehículo.</li>
              <li>Cédula de identificación del vehículo (Cédula Verde o Cédula Azul/Autorizado).</li>
              <li>Comprobante de seguro obligatorio automotor vigente.</li>
              <li>Constancia de Revisión Técnica Obligatoria (RTO / VTV) al día.</li>
              <li>Comprobante de pago de patente al día.</li>
            </ul>
          `
        },
        {
          id: 'ch3',
          title: '3. Elementos de Seguridad en el Auto',
          content: `
            <h4>🧯 Equipamiento Indispensable a Bordo:</h4>
            <ul>
              <li>Matafuegos con carga vigente (1 kg), al alcance del conductor y con soporte fijo.</li>
              <li>Juego de dos balizas portátiles reflectantes triangulares.</li>
              <li>Rueda de auxilio en óptimas condiciones de inflado y llave cruz con crique.</li>
              <li>Chaleco reflectivo recomendado para descensos en banquina.</li>
            </ul>
          `
        }
      ]
  ...
  function openManualViewer(manualKey) {
    const data = MANUALS_DATABASE[manualKey];
    if (!data || !modalManualViewer) return;

    if (manualViewTag) manualViewTag.textContent = data.tag;
    if (manualViewTitle) manualViewTitle.textContent = data.title;

    if (manualChaptersNav) {
      manualChaptersNav.innerHTML = data.chapters.map((ch, idx) => `
        <button type="button" class="manual-ch-btn ${idx === 0 ? 'active' : ''}" data-ch-idx="${idx}">
          ${ch.title}
        </button>
      `).join('');

      manualChaptersNav.querySelectorAll('.manual-ch-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          manualChaptersNav.querySelectorAll('.manual-ch-btn').forEach(b => b.classList.remove('active'));
          btn.classList.add('active');
          const idx = parseInt(btn.dataset.chIdx, 10);
          renderManualChapter(data.chapters[idx]);
        });
      });
    }

    renderManualChapter(data.chapters[0]);

    modalManualViewer.style.display = 'flex';
    modalManualViewer.classList.add('active');
  }

  function renderManualChapter(chapter) {
    if (!manualViewContent || !chapter) return;
    manualViewContent.innerHTML = chapter.content;
    manualViewContent.scrollTop = 0;
  }

  btnCloseManualView?.addEventListener('click', () => {
    if (modalManualViewer) {
      modalManualViewer.style.display = 'none';
      modalManualViewer.classList.remove('active');
    }
  });

  document.querySelectorAll('.btn-hub-read-manual, .hub-manual-item').forEach(el => {
    el.addEventListener('click', (e) => {
      e.stopPropagation();
      const manualKey = el.dataset.manual;
      if (manualKey) openManualViewer(manualKey);
    });
  });
